...
export async function handleReservation(request,endpoint){
 ...
 if(cidade.length<2||cidade.length>100||data.website||nome.length<2||nome.length>100||!/^\d{10,15}$/.test(telefone)||email.length>160||!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)||data.consent!==true)return json({ok:false},400);
 if(!endpoint)return json({ok:false},503);
 try{
  const result=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({nome,telefone,email,cidade,consent:true}),signal:AbortSignal.timeout(20000)});
